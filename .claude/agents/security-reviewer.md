...
description: Read-only reviewer of the repository's security posture — workflow permissions and token scope, untrusted input reaching shells, the ruleset, and the scanning setup. Reports findings, never edits. Launched by /repo-review.
...
`.github/workflows/*.yml`, `.github/rulesets/*.json`, `.github/renovate.json5`, the scripts under `scripts/` and `.github/scripts/`, `.claude/settings.json`, and the documents that tell the user how to create the secrets (`docs/drift-check.md`, `docs/renovate.md`, `SECURITY.md`).
...
- **The permission lists.** Read `.claude/settings.json` against what `README.md` says it keeps out of reach: a credential path the `deny` list does not cover, and the difference between `deny`, which governs the tools, and `sandbox.credentials`, which governs sandboxed commands. The `allow` list is empty, so every command goes through a prompt; report an entry added to it that reaches further than the command it was written for. Nothing here enforces the destructive git commands `CLAUDE.md` forbids — report a rule left without enforcement, but a spelling that only a shell expansion produces is out of reach of a pattern and is not a finding.
