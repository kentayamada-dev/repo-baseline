...
- **The permission lists.** Read the `deny` list in `.claude/settings.json` against the rules it stands for in `CLAUDE.md`: a destructive git command whose plain spelling is not named, and an `allow` entry whose pattern reaches further than the read-only command it was written for. A spelling that only a shell expansion produces is out of reach of a pattern and is not a finding.
