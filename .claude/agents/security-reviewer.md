...
- **The git guards in `.claude/hooks/`.** Try to defeat each deny hook on paper: alternative spellings (`--force-with-lease`, `+refspec`, short flags bundled with others, `git -c` options, `git -C`), commands wrapped in `sh -c`, `xargs`, `env`, aliases, subshells, or chained after `;`, `&&`, `|`. Report each bypass with the exact command that gets through.
