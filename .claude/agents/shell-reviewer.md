...
- **Hooks specifically.** The hook reads the tool call as JSON on stdin and answers on stdout, so work through the calls it was not written for: a field that is missing, `null`, or not a string, stdin that is not JSON at all, and an environment without `CLAUDE_PROJECT_DIR`. A `PostToolUse` hook runs after the edit, so failing there costs the call without preventing anything — check that it stays silent instead. Then check the match in both directions: a file the hook should answer for and passes over, and one it answers for that the rule does not cover.
