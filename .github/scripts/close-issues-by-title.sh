...
# An empty title matches no issue, so the run would close nothing and still succeed: a
# caller whose TITLE went missing looks green while the issue it meant to retract stays
# open. upsert-issue.sh turns an empty --title down for the same reason.
