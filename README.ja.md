...
| [CLAUDE.md](CLAUDE.md) | Claude Code が読み込む指示書。下のテンプレートで置き換える |
| [CLAUDE.template.ja.md](CLAUDE.template.ja.md) | 自分の CLAUDE.md を書くためのテンプレート（[English](CLAUDE.template.md)） |
| [mise.toml](mise.toml) | CI とローカルで使う検査ツールのバージョン |
...
日本語にしてあるのは README と `docs/` のドキュメントだけで、正となるのは英語版です。スクリプトやワークフローの出力、issue / PR のテンプレートは英語のみなので、`--check` が出す `OK` / `DRIFT` / `UNKNOWN` や自動で立つ通知 issue のタイトルは、この文書でも英語のまま引用しています。
...
コミットの署名には手元の準備が必要です。ruleset の対象は main だけなので作業ブランチへの push は未署名でも通りますが、ルールが見るのは PR に含まれるすべてのコミットで、マージ時に GitHub が作って署名する squash コミットだけではありません。未署名のコミットを含む PR は、その PR の作成者自身でもマージできません。GPG か SSH の署名鍵を用意し（[コミット署名の検証を管理する](https://docs.github.com/ja/authentication/managing-commit-signature-verification)）、既定で署名するようにします。
...
editorconfig-checker と shfmt は、他の検査ツールと同じく本体を mise で入れて実行しています（[ツールの導入と検証](docs/ci-jobs.ja.md#ツールの導入と検証)、バージョンは [mise.toml](mise.toml)）。手元では次で同じ検査を実行できます。**editorconfig-checker のコマンド名は `ec` です**（[mise.toml](mise.toml) に書く `editorconfig-checker` は [aqua](https://aquaproj.github.io/) レジストリでのパッケージ名で、バイナリの名前とは違います）。
...
`shfmt -d` を `shfmt -w` に変えると、差分を出す代わりにその場で整形します。
...
手元では次で同じ検査を実行できます。`config.yml` はフォームではなくフォームの選択画面の設定で、スキーマが別なので分けて検査します。拡張子は GitHub が `.yml` と `.yaml` のどちらも受け付けるため、両方を対象にしています。
