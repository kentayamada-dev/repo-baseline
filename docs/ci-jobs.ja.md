...
| `translation-sync` | 対訳の両言語が一緒に変更されているか | [translation-sync](#translation-sync) |
...
| `setup-script` | セットアップスクリプトの実行確認 | [setup-script](#setup-script) |
...
検査に使う CLI ツールは、本体を [mise](https://mise.jdx.dev/) で入れて実行しています。公式の action や Docker イメージがあるツールでも使いません。バージョンの情報源を [mise.toml](../mise.toml) 1 つにまとめ、手元でも CI と同じものを動かせるようにするためです。更新は [Renovate](renovate.ja.md#renovate) が PR で提案します（mise マネージャが標準で対応）。
...
手元に mise を入れて次を実行すれば、CI と同じバージョンが揃います。各ツールの節の実行例は、これを済ませてある前提です。
...
本体は mise で入れています（[ツールの導入と検証](#ツールの導入と検証)）。**このジョブでは shellcheck も一緒に入れています。** actionlint は shellcheck が PATH に無いと `run:` の検査を黙って飛ばすためです。同様に、`run:` に Python を書くようになったら pyflakes を [mise.toml](../mise.toml) と `install_args` に足してください。
...
アンカーの照合は、lychee が見出しから生成する ID で行います。生成規則は GitHub の描画と一致し、日本語の見出しもそのまま ID になります（`#### 2 つの例外` → `#2-つの例外`。同じ文言の見出しには 2 つ目以降に `-1`、`-2` が付きます）。

本体は mise で入れています（[ツールの導入と検証](#ツールの導入と検証)。lychee のリリースに付く `.sha256` は aqua レジストリ側に設定が無いため、mise の検証は掛かりません）。手元でも上と同じコマンドで走らせます。

### 外部リンクの定期検査

外部 URL は [link-check.yml](../.github/workflows/link-check.yml) が毎日（08:00 JST）と main への push 時に確認します。同じ lychee ですが、役割が違います。
...
| `MD013`（行の長さ） | 本文 1000 文字、コードブロック 120 文字、見出しと表は無効 | 本文は折り返さず 1 段落 1 行で書くため（すぐ下）。コードブロックは折り返せず、長いと横スクロールになる |
...
手元では npm 経由で走らせます。バージョンは action が同梱しているものに合わせてください（[ci.yml](../.github/workflows/ci.yml) の action のタグと、その [package.json](https://github.com/DavidAnson/markdownlint-cli2-action/blob/main/package.json) の `dependencies` を見ます）。`--fix` を付けると直せる指摘はその場で直せます。
...
[ci.yml](../.github/workflows/ci.yml) の `translation-sync` ジョブが、文書とその翻訳が一緒に変更されているかを検査します。[README.md](../README.ja.md) と `docs/` 配下は二言語で、`X.md` が英語の原文、`X.ja.md` が日本語の訳です（[二言語の文書](../CONTRIBUTING.md#bilingual-documentation)）。片方だけ編集しても何も落ちないため、このジョブが無いと訳が黙って古くなります。[設定のずれの検査](drift-check.ja.md#設定のずれの検査)と同じ失敗の型で、対象が設定ではなく文書というだけです。

git 管理下の `*.ja.md` を同名の `*.md` と対にして、PR が片方だけを触っていたら落とします。**内容の一致は見ません。** 分かるのは「片方を変えた人がもう片方を開いた」ことだけで、文言を揃える責任は書いた人に残ります。

| 状況 | 結果 |
| --- | --- |
| 両方が変更されている | 通る |
| 片方だけ変更されている | 落ちる（取り残された側のファイル名を出します） |
| 同名の `*.md` が無い `*.ja.md` | 落ちる（対が単位なので、原文の無い訳は検査できません） |
| どちらも変更されていない | 通る（見るのは PR が触ったものだけです） |
| PR 本文に `translation-not-needed` がある | 通る（下記） |

比較する base のコミットが必要なので `pull_request` でのみ走ります。main への push ではスキップされ、`ci` では成功として扱われます。`actions/checkout` に `fetch-depth: 0` を付けているのは、既定の深さ 1 では base のコミットが入らないためです。

英語の誤字直しや、訳が既に正しく表している文の言い換えなど、本当に対応が要らない変更のときは、PR 本文のどこかに `translation-not-needed` と書いてください。ラベルではなく本文のマーカーにしてあるのは、ラベルだとセットアップスクリプトで作って `LABELS_EXPECTED` で追跡する必要があり（[ラベル](../README.ja.md#ラベル)）、このリポジトリの他のラベルはすべて手で付けずに自動で付くものだからです。

この逃げ道に検査を付けていないのは意図的です。このジョブは承認の関門ではなく注意喚起で、回避できない注意喚起は「もう片方に無意味な変更を入れる」で迂回されます。そちらの方が悪く、翻訳を更新していないのに更新したと履歴が主張することになります。
...
主な検査項目です（[全ポリシー](https://github.com/suzuki-shunsuke/ghalint#policies)）。

| ポリシー | 内容 |
| --- | --- |
| `job_permissions` | 全ジョブに `permissions` を書く（ワークフロー全体が `{}` のときは省略可） |
| `job_timeout_minutes_is_required` | 全ジョブに `timeout-minutes` を書く |
| `checkout_persist_credentials_should_be_false` | `actions/checkout` に `persist-credentials: false` を付ける |
| `action_ref_should_be_full_length_commit_sha` | action の参照を 40 桁の commit SHA にする |
| `deny_inherit_secrets` / `workflow_secrets` / `job_secrets` | `secrets: inherit` と、ワークフロー / ジョブの env への secret の設定を禁じる |
...
本体は mise で入れています（[ツールの導入と検証](#ツールの導入と検証)）。手元では `ghalint run` で走らせます。
...
主な監査項目です（[全項目](https://docs.zizmor.sh/audits/)）。

| 監査 | 内容 |
| --- | --- |
| `template-injection` | `run:` に `${{ }}` を直接埋め込んでいないか（第三者が書ける値がシェルに渡ると任意コード実行になる） |
| `dangerous-triggers` | `pull_request_target` のような、fork の PR を権限付きで走らせる trigger を使っていないか |
| `excessive-permissions` | ジョブに必要以上の `permissions` を与えていないか |
| `unpinned-uses` / `unpinned-images` | action と Docker イメージがダイジェストで固定されているか |
| `impostor-commit` | 固定した SHA が、その action 本体ではなく fork にしか無いコミットではないか |
| `known-vulnerable-actions` | 使っている action に既知の脆弱性が報告されていないか |
| `artipacked` | `actions/checkout` がトークンを `.git/config` に残していないか |

`impostor-commit` と `known-vulnerable-actions` は GitHub API を使うため、トークンが無いと黙ってスキップされます。ジョブでは `GITHUB_TOKEN` に `github.token` を渡して有効にしています（公開情報しか参照しないので `contents: read` で足ります）。
...
```bash
gitleaks git --redact --verbose    # 履歴全体（CI と同じ）
gitleaks dir --redact --verbose .  # 現在のファイルだけ
```

## setup-script

[ci.yml](../.github/workflows/ci.yml) の `setup-script` ジョブが、[セットアップ](../README.ja.md#セットアップ)のスクリプトを `--dry-run` で実際に実行します。[shellcheck](#shellcheck) は静的な検査なので、変数名の取り違えなどは通ってしまいます。このスクリプトを動かすのはリポジトリを作った直後の 1 回だけで、壊れていても気づくのはテンプレートからリポジトリを作った人です。唯一の実行経路を CI で通しておきます。

`--dry-run` は読み取りだけで完結し、何も変更しません。`.github/rulesets/*.json` が壊れていればここで落ちるため、ruleset の JSON の構文ミスも PR で捕まります。あわせて `--help` の出力が空でないことも確かめています（ヘルプはスクリプト冒頭のコメントを awk で切り出しているため、コメントを消すと黙って空になります）。

`gh` と `jq` は GitHub ホストの runner に最初から入っているため、[mise.toml](../mise.toml) には足していません。認証はワークフローの `GITHUB_TOKEN` で足ります。

private リポジトリではこのジョブを走らせません（スクリプトが public 以外を拒否するため、回せば必ず落ちます）。スキップは `ci` 側で成功扱いになるので、private でも PR は止まりません。
...
- 手元で同じ検査を再現できません（[手元で走らせる](#手元で走らせる)）。
...
本体は [mise.toml](../mise.toml) に入れていないので、必要なときだけバージョンを指定して `mise x` で走らせます。

```bash
mise x osv-scanner@<バージョン> -- osv-scanner scan source --recursive .
```

CI と同じバージョンにはなりません（CI 側は呼び出し先のワークフローが持つバージョンで動きます）。差分検査の「base との比較」も再現できません。手元で分かるのは、いまの作業ツリーに既知の脆弱性を持つ依存が入っているかどうかまでです。

### 問い合わせ先

既定ではパッケージ名とバージョンを [api.osv.dev](https://osv.dev) へ送って照合します（ソースコードは送りません）。外部へ何も出したくない場合は、脆弱性データベースを手元に落として照合する `--offline-vulnerabilities`（初回の取得は `--download-offline-databases`）を `scan-args` に足します。
