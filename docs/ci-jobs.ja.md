...
`jq` は GitHub ホストの runner に最初から入っていて、そもそも呼ぶのはフック自身なので、[mise.toml](../mise.toml) に足すのは bats だけです。
...
[.github/scripts/tests/](../.github/scripts/tests) には、隣のスクリプトを見ないテストが 1 つあります。Conventional Commits の type 一覧のコピーです。どれも他から導出されないため、[PR タイトルの書式](../README.ja.md#pr-タイトルの書式)に挙げた各コピー（[ci.yml](../.github/workflows/ci.yml) 内の失敗メッセージも含む）を、`pr-title` ジョブが実際に強制する `PATTERN` と突き合わせます。ここに置いてあるのは、テンプレートから作ったリポジトリが消してよい `.claude/` より長く残るからです。
