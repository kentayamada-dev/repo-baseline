...
| `translation-sync` | Whether both languages of a document changed together | [translation-sync](#translation-sync) |
...
| `setup-script` | That the setup script actually runs | [setup-script](#setup-script) |
...
The CLI tools used for the checks are installed with [mise](https://mise.jdx.dev/) and run directly, even for tools that have an official action or Docker image. That keeps [mise.toml](../mise.toml) as the single source of truth for versions and lets you run exactly what CI runs locally. Updates are proposed as PRs by [Renovate](renovate.md#renovate) (the mise manager is supported out of the box).
...
Install mise locally and run the following to get the same versions CI uses. The example invocations in each tool's section assume you have done this.
...
The tool is installed with mise ([Installing and verifying the tools](#installing-and-verifying-the-tools)). **This job installs shellcheck alongside it**, because actionlint silently skips checking `run:` when shellcheck is not on PATH. Likewise, once you start writing Python in `run:`, add pyflakes to [mise.toml](../mise.toml) and to `install_args`.
...
The tool is installed with mise ([Installing and verifying the tools](#installing-and-verifying-the-tools); the `.sha256` attached to lychee releases has no configuration in the aqua registry, so mise's verification does not apply). Locally, run the same command as above.

### Scheduled external link checks

External URLs are checked by [link-check.yml](../.github/workflows/link-check.yml) daily (08:00 JST) and on pushes to main. It is the same lychee, in a different role.
...
Locally, run it through npm. Match the version to the one the action bundles (look at the action's tag in [ci.yml](../.github/workflows/ci.yml) and at `dependencies` in its [package.json](https://github.com/DavidAnson/markdownlint-cli2-action/blob/main/package.json)). Adding `--fix` fixes the findings that can be fixed automatically.
...
The `translation-sync` job in [ci.yml](../.github/workflows/ci.yml) checks that a document and its translation changed together. [README.md](../README.md) and the files under `docs/` are bilingual: `X.md` is the English original and `X.ja.md` is the Japanese translation ([Bilingual documentation](../CONTRIBUTING.md#bilingual-documentation)). Nothing about editing one of them makes the other fail, so without this job the translation silently falls behind — the same failure mode as the [settings drift check](drift-check.md#settings-drift-check), one document instead of one setting.

The job pairs up every `*.ja.md` under git with the `*.md` of the same name, and fails when the PR touched one side of a pair and not the other. **It does not compare meaning.** All it proves is that whoever changed one file opened the other; keeping the wording in step is on the author.

| Situation | Result |
| --- | --- |
| Both sides changed | Passes |
| Only one side changed | Fails, naming the file that was left behind |
| A `*.ja.md` with no `*.md` of the same name | Fails (the pair is the unit; a translation of nothing cannot be checked) |
| Neither side changed | Passes (the job only looks at what the PR touched) |
| The PR body contains `translation-not-needed` | Passes (see below) |

It runs on `pull_request` only, because the check needs a base commit to diff against. On a push to main it is skipped, which counts as a success in `ci`. `fetch-depth: 0` is set on `actions/checkout` because the default depth of 1 does not include the base commit.

For a change that genuinely needs no counterpart — fixing an English typo, rewording a sentence the translation already renders correctly — write `translation-not-needed` anywhere in the PR body. That is deliberately a marker in the body rather than a label: a label would have to be created by the setup script and tracked in `LABELS_EXPECTED` ([Labels](../README.md#labels)), and every other label in this repository is applied automatically rather than by hand.

The escape hatch is unaudited on purpose. This job is a reminder, not an approval gate, and a reminder that cannot be waived gets worked around by making a no-op edit to the other file — which is worse, because it makes the history claim a translation was updated when it was not.
...
The main checks ([all policies](https://github.com/suzuki-shunsuke/ghalint#policies)):

| Policy | Description |
| --- | --- |
| `job_permissions` | Write `permissions` on every job (it may be omitted when the workflow-wide value is `{}`) |
| `job_timeout_minutes_is_required` | Write `timeout-minutes` on every job |
| `checkout_persist_credentials_should_be_false` | Add `persist-credentials: false` to `actions/checkout` |
| `action_ref_should_be_full_length_commit_sha` | Reference actions by a 40-character commit SHA |
| `deny_inherit_secrets` / `workflow_secrets` / `job_secrets` | Forbid `secrets: inherit` and setting secrets in a workflow-level or job-level env |
...
The tool is installed with mise ([Installing and verifying the tools](#installing-and-verifying-the-tools)). Locally, run it with `ghalint run`.
...
The main audits ([all of them](https://docs.zizmor.sh/audits/)):

| Audit | Description |
| --- | --- |
| `template-injection` | Whether `${{ }}` is embedded directly in `run:` (a value a third party can write reaching the shell means arbitrary code execution) |
| `dangerous-triggers` | Whether a trigger such as `pull_request_target`, which runs a fork's PR with privileges, is used |
| `excessive-permissions` | Whether a job is given more `permissions` than it needs |
| `unpinned-uses` / `unpinned-images` | Whether actions and Docker images are pinned by digest |
| `impostor-commit` | Whether a pinned SHA is a commit that exists only in a fork rather than in the action itself |
| `known-vulnerable-actions` | Whether an action in use has a known vulnerability reported |
| `artipacked` | Whether `actions/checkout` leaves a token in `.git/config` |

`impostor-commit` and `known-vulnerable-actions` use the GitHub API and are silently skipped without a token. The job passes `github.token` as `GITHUB_TOKEN` to enable them (`contents: read` is enough, since only public information is referenced).
...
```bash
gitleaks git --redact --verbose    # the entire history (the same as CI)
gitleaks dir --redact --verbose .  # only the current files
```

## setup-script

The `setup-script` job in [ci.yml](../.github/workflows/ci.yml) actually runs the script from [Setup](../README.md#setup) with `--dry-run`. [shellcheck](#shellcheck) is a static check, so a mistyped variable name passes it. This script is run exactly once, right after the repository is created, and if it is broken the person who finds out is whoever created a repository from the template. Its one and only execution path is exercised in CI.

`--dry-run` is read-only and changes nothing. A broken `.github/rulesets/*.json` fails here, so a JSON syntax error in a ruleset is caught on the PR as well. It also confirms that the `--help` output is not empty (the help is carved out of the comment at the top of the script with awk, so removing the comment silently empties it).
...
This job does not run on a private repository (the script refuses anything but public, so running it would always fail). A skip counts as a success in `ci`, so PRs are not blocked on a private repository either.
...
- The same check cannot be reproduced locally ([Running it locally](#running-it-locally)).
...
The tool is not in [mise.toml](../mise.toml), so run it with `mise x` and an explicit version when you need it.

```bash
mise x osv-scanner@<version> -- osv-scanner scan source --recursive .
```

It will not be the same version as CI (the CI side runs whatever version the called workflow holds). The diff scan's "comparison against the base" cannot be reproduced either. All you learn locally is whether the current working tree contains a dependency with a known vulnerability.

### Where queries are sent

By default the package names and versions are sent to [api.osv.dev](https://osv.dev) for matching (the source code is not sent). If you want nothing to leave your machine, add `--offline-vulnerabilities` (with `--download-offline-databases` for the first fetch) to `scan-args` to match against a locally downloaded vulnerability database.
