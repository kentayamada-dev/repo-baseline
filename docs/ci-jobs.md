...
The `hooks` job in [ci.yml](../.github/workflows/ci.yml) runs the [bats](https://bats-core.readthedocs.io/) tests in [.claude/tests/](../.claude/tests) against the hook scripts next to them in [.claude/hooks/](../.claude/hooks). A hook is a filter — the tool call arrives as JSON on stdin, the answer leaves as JSON on stdout — so one test feeds one tool call and reads the answer back. The command is the `check:hooks` task in [mise.toml](../mise.toml).

What is left to a hook is narrow, and deliberately so. The ruleset in [main.json](../.github/rulesets/main.json) refuses a non-fast-forward push to main and every push that does not arrive through a pull request, with no bypass actor, so a force push and a commit landing on main are already impossible where they would do harm; a malformed PR title cannot be merged either, since `pr-title` is part of the required check. A hook in front of either of those is a second lock on a door that is already held shut. What is left to [remind-bilingual-pair.sh](../.claude/hooks/remind-bilingual-pair.sh) is the rule no permission can name, because what decides it is the file that was written rather than the tool that wrote it.

What the tests are for is the borders, not the obvious cases. The path is judged relative to the project directory, so the README of the repository is one half of a pair while `scripts/README.md` is not, `READMEs.md` is not the README at all, and a file outside the project is none of the hook's business; with no `CLAUDE_PROJECT_DIR` to be relative to, the answer is silence rather than a guess, and the README at the filesystem root is the trap that would otherwise slip through. The rest is what the hook must not do: it runs after the edit, so a tool call it can say nothing about — no file path, a path that is not text, stdin that is not JSON — has to be left alone rather than failed.
...
`jq` ships with GitHub-hosted runners and is what the hook itself calls, so bats is the only addition to [mise.toml](../mise.toml).
...
One test in [.github/scripts/tests/](../.github/scripts/tests) answers to no script beside it: the copies of the Conventional Commits type list. Nothing derives them from one another, so the test compares each copy named in [PR title format](../README.md#pr-title-format) — the failure message in [ci.yml](../.github/workflows/ci.yml) included — against the `PATTERN` the `pr-title` job actually enforces. It sits here because it outlives `.claude/`, which the repository built from this template is free to delete.
