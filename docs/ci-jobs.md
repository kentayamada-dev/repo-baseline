...
What is left to a hook is narrow, and deliberately so. The ruleset in [main.json](../.github/rulesets/main.json) refuses a non-fast-forward push to main and every push that does not arrive through a pull request, with no bypass actor, so a force push and a commit landing on main are already impossible where they would do harm; a malformed PR title cannot be merged either, since `pr-title` is part of the required check; and the destructive git commands are named outright by the `deny` list in [.claude/settings.json](../.claude/settings.json). A hook in front of any of those is a second lock on a door that is already held shut. What is left to [remind-bilingual-pair.sh](../.claude/hooks/remind-bilingual-pair.sh) is the rule no permission can name, because what decides it is the file that was written rather than the tool that wrote it.
