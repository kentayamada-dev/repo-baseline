...
What the tests are for is the borders, not the obvious cases: a commit is denied on main and allowed once the working branch is a command of its own, and `git log | grep push` is not a push. Those borders are matched textually rather than by parsing the shell, over a command each hook normalizes first — a backslash-continued line is folded onto one, since there the newline separates tokens rather than commands, and the quote characters are dropped, since a quoted `"-f"` reaches git as the flag `-f` — and what is left is confined to one pipeline segment by `[^|;&\n]`, a plain newline separating commands just as `;` does.

Normalizing that far and no further leaves one shape unresolved: a flag the shell produces only by expanding something (`h=--hard; git reset $h`). A textual match never learns what that becomes, so a push or reset carrying a `$` or a backtick is asked about rather than judged — unless the command already names a flag that settles the question, since `git reset --soft $BASE` cannot turn into a hard reset. That tier is asserted like the false positives the textual match brings with it (a quoted `echo "git commit"` is denied all the same) — a failure in either means the behaviour moved, not that it improved.

Those false positives stop at one border, which is asserted too: a keyword right after a hyphen or a slash belongs to a name rather than being a subcommand, so `git show HEAD:.claude/hooks/deny-commit-on-main.sh` reads a file and is left alone. The hooks are named after what they guard, and a guard that cannot be read is one that cannot be maintained.
