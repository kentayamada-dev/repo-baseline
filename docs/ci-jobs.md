...
What is left to a hook is narrow, and deliberately so. The ruleset in [main.json](../.github/rulesets/main.json) refuses a non-fast-forward push to main and every push that does not arrive through a pull request, with no bypass actor, so a force push and a commit landing on main are already impossible where they would do harm; a malformed PR title cannot be merged either, since `pr-title` is part of the required check. A local guard in front of any of those is a second lock on a door the server already holds shut. What no ruleset reaches is the working tree of this machine, and that is the whole remit of [deny-hard-reset-clean.sh](../.claude/hooks/deny-hard-reset-clean.sh): a hard reset and a `git clean` delete work that was never pushed anywhere, so nothing can hand it back.

What the tests are for is the borders, not the obvious cases: `--hard` is denied wherever it sits in the command, and `git status --porcelain | grep clean` is not a clean. Those borders are matched textually rather than by parsing the shell, over a command the hook normalizes first — a backslash-continued line is folded onto one, since there the newline separates tokens rather than commands, and the quote characters are dropped, since a quoted `"--hard"` reaches git as the flag `--hard` — and what is left is confined to one pipeline segment by `[^|;&\n]`, a plain newline separating commands just as `;` does.

Normalizing that far and no further leaves one shape unresolved: a flag the shell produces only by expanding something (`h=--hard; git reset $h`). A textual match never learns what that becomes, so a reset carrying a `$` or a backtick is asked about rather than judged — unless the command already names a mode that settles the question, since `git reset --soft $BASE` cannot turn into the destructive one. That tier is asserted like the false positives the textual match brings with it (a quoted `echo "git clean -fdx"` is denied all the same) — a failure in either means the behaviour moved, not that it improved.

Those false positives stop at one border, which is asserted too: a keyword right after a hyphen or a slash belongs to a name rather than being a subcommand, so `git show HEAD:.claude/hooks/deny-hard-reset-clean.sh` reads a file and is left alone. The hook is named after what it guards, and a guard that cannot be read is one that cannot be maintained.
...
So are the copies of the Conventional Commits type list. Nothing derives them from one another, so the tests compare each copy named in [PR title format](../README.md#pr-title-format) against the `PATTERN` — the one the `pr-title` job actually enforces.

`jq` ships with GitHub-hosted runners and is what the hooks themselves call, so bats is the only addition to [mise.toml](../mise.toml).
