...
**secret に `RENOVATE_TOKEN` を登録するまで動きません**（未登録ならジョブがその旨を出して落ちます）。発行するのは fine-grained PAT です。PAT を発行する API は無いので、作成はブラウザで行います。
...
2. 次の内容で作成する
    ...
    | 項目 | 値 |
    | --- | --- |
    | Token name | `renovate (OWNER/REPO)` など、用途が分かる名前 |
    | Resource owner | リポジトリの所有者（Organization の場合は組織側の承認が必要なことがあります） |
    | Expiration | 運用に合わせて。切れると実行が失敗し、issue が立ちます（[実行が失敗したとき](#実行が失敗したとき)） |
    | Repository access | Only select repositories → 対象のリポジトリ |

3. Repository permissions で以下を与える（表にない権限は不要です）
...
5. 実際に動かして確認する（更新があれば PR が作られます）
...
| | 動作 |
| --- | --- |
| 落ちたとき | `Renovate runs are failing` という issue を作る。実行ログの URL と直し方を本文に入れます |
| 作った issue | `maintenance` ラベルを付けます（[ラベル](../README.ja.md#ラベル)） |
| すでに同じ issue が open のとき | 何もしません（毎週同じ issue が積み上がらないように） |
...
**schedule の自動停止はこの通知では拾えません。** リポジトリの活動が 60 日間無いと GitHub が schedule を止めますが、実行自体が起きないため issue も立ちません。GitHub からの停止通知メールが唯一の手掛かりです（[renovate.yml](../.github/workflows/renovate.yml) のコメントを参照）。
...
| | 動作 |
| --- | --- |
| 引けなかった依存があるとき | `Some dependencies cannot be resolved` という issue を作る。Renovate が出した警告と実行ログの URL を本文に入れます |
| 作った issue | `maintenance` ラベルを付けます（[ラベル](../README.ja.md#ラベル)） |
| すでに同じ issue が open のとき | 本文を書き換えます（引けなかった依存は実行ごとに変わるため） |
| すべて引けるようになったとき | その issue を自動的に閉じます |
...
| | 動作 |
| --- | --- |
| 更新 PR があるとき | `Dependency updates are available` という issue を作る。PR の一覧と実行ログの URL を本文に入れます |
| 作った issue | `dependencies` ラベルを付けます（[ラベル](../README.ja.md#ラベル)） |
| すでに同じ issue が open のとき | 本文を書き換えます（内容が実行ごとに変わるため） |
| 更新 PR が無くなったとき | その issue を自動的に閉じます |
...
| 設定 | 理由 | 外すと |
| --- | --- | --- |
| `semanticCommits: 'enabled'` | PR タイトルを `chore(deps): ...` にして [`pr-title`](../README.ja.md#pr-タイトルの書式) を通す | 既定の `auto`（履歴からの自動判定）になり、判定が外れると `pr-title` が落ちてマージできない |
