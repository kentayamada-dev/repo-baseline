...
**途中で止まった実行はもう一度走らせ、それでも止まったときだけ `A Renovate run stopped partway` という issue が立ちます**（再実行は `renovate` ジョブ、issue は [renovate.yml](../.github/workflows/renovate.yml) の `abort` ジョブ）。Renovate は実行中にリポジトリが動くと残りを諦めます。push が古い情報として弾かれた、見ていたブランチやコミットが期待した場所に無い、といった状況です。原因はたいてい、実行中にマージが入ったことです。更新 PR のマージでも起きます。[セットアップ](../README.ja.md#セットアップ)でマージ後のブランチ自動削除を有効にしているため、実行中の Renovate が見ているブランチがマージで消えるためです。手が回っていなかった分はそのままです。そこにあった更新の PR は作られず、既にある PR も更新されません。

これは失敗の形をしていません。Renovate は `Repository has changed during renovation - aborting` を info レベルで出し、終了コード 0 で終わります（終了コードが 0 以外になるのは error 以上のログが出たときだけです）。そのため `renovate` ジョブは成功し、`notify` は成功と見て自分の issue を閉じ、`dashboard` はそのとき存在する PR を並べます。実行は週 1 回なので、放っておけば次の機会は 1 週間後です。

2 回目は途中から再開するのではなく clone からやり直すので、その時点のリポジトリを見て動きます。1 回目を止めた変更はもう終わっています。1 回止まっただけで issue を立てないのはこのためで、代わりに run に警告の注記が残ります。どのくらいの頻度で起きているかはそれを見ます。

issue は `maintenance` ラベル付きで立ち、本文に実行ログの URL と、リポジトリが落ち着いてから叩く 1 コマンドが入ります。すでに同じ issue が open の間は本文を書き換え（意味を持つのは最新の実行だけなので）、止まらずに終わる実行があれば自動的に閉じます。

仕組みは[依存を解決できなかったとき](#依存を解決できなかったとき)と同じ実行ログの拾い読みで、再実行も issue もこのメッセージ 1 つに乗っています。Renovate 側の文言が変わるとどちらも起きなくなり、止まった実行はまた静かに終わるようになります。
