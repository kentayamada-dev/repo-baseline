...
**Nothing happens until the `RENOVATE_TOKEN` secret is registered** (if it is not, the job says so and fails). The token to issue is a fine-grained PAT. There is no API for issuing a PAT, so create it in the browser.
...
2. Create it with the following
    ...
    | Item | Value |
    | --- | --- |
    | Token name | A name that says what it is for, such as `renovate (OWNER/REPO)` |
    | Resource owner | The repository owner (for an Organization, approval on the organization side may be required) |
    | Expiration | Whatever suits how you operate. Once it expires the run fails and an issue is opened ([When a run fails](#when-a-run-fails)) |
    | Repository access | Only select repositories → the target repository |

3. Under Repository permissions grant the following (nothing outside this table is needed)
...
5. Run it for real to confirm (a PR is created if there is an update)
...
| | Behavior |
| --- | --- |
| On failure | Creates an issue titled `Renovate runs are failing`. The body holds the run log URL and how to fix it |
| The issue it creates | Gets the `maintenance` label ([Labels](../README.md#labels)) |
| When the same issue is already open | Does nothing (so the same issue does not pile up weekly) |
...
**The automatic disabling of the schedule cannot be caught by this notification.** GitHub disables the schedule after 60 days without repository activity, and since no run happens, no issue is opened either. GitHub's deactivation email is the only clue (see the comment in [renovate.yml](../.github/workflows/renovate.yml)).
...
| | Behavior |
| --- | --- |
| When some dependency could not be resolved | Creates an issue titled `Some dependencies cannot be resolved`. The body holds the warning Renovate emitted and the run log URL |
| The issue it creates | Gets the `maintenance` label ([Labels](../README.md#labels)) |
| When the same issue is already open | Rewrites the body (which dependencies fail changes from run to run) |
| Once everything resolves | Closes that issue automatically |
...
| | Behavior |
| --- | --- |
| When update PRs exist | Creates an issue titled `Dependency updates are available`. The body holds the list of PRs and the run log URL |
| The issue it creates | Gets the `dependencies` label ([Labels](../README.md#labels)) |
| When the same issue is already open | Rewrites the body (the contents change from run to run) |
| When no update PRs remain | Closes that issue automatically |
