...
# returned for it ($2, JSON), one per line, and prints nothing when they agree.
#
# Only what the definition names is compared. The API adds fields of its own (id,
# created_at, and parameters GitHub introduces later), and reporting those would turn
# every addition on GitHub's side into drift. Arrays are sorted first because the API
# does not promise to hand back the order they were sent in.
ruleset_diff() {
...
check_settings() {
  local current key gql want got name kv endpoint fields i id ruleset_now diff_lines line
...
The default GITHUB_TOKEN in GitHub Actions cannot read the merge-related settings
(allow_* and squash_merge_commit_title), immutable releases, secret scanning push
protection, or the default permissions of the Actions GITHUB_TOKEN. Register a PAT with
Administration read access as the SETTINGS_TOKEN secret (see docs/drift-check.md).
MSG
  fi
  exit 1
fi
